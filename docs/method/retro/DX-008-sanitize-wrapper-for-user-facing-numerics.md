---
title: "DX-008 — Sanitize Wrapper for User-Facing Numeric Inputs"
legend: DX
lane: retro
---
...
Completed on `release/v5.0.0` as the shared numeric sanitizer module in
`packages/bijou/src/core/numeric.ts`. The repo now exports reusable finite and
integer boundary helpers, uses them in shared layout/runtime/components code,
and covers the wrapper behavior in `packages/bijou/src/core/numeric.test.ts`.

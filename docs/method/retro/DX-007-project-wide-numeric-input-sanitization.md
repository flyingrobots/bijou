---
title: "DX-007 — Project-Wide Numeric Input Sanitization"
legend: DX
lane: retro
---
...
Completed on `release/v5.0.0`. Bijou now has shared numeric boundary helpers in
`packages/bijou/src/core/numeric.ts`, broad adoption across core, TUI, and
Node adapters, and dedicated tests in `packages/bijou/src/core/numeric.test.ts`.
What was originally an audit reminder is now a reusable boundary rule rather
than repeated ad hoc guards.

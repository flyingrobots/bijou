...
This cycle exists to put that truth directly on the landing screen:
...
- a landing-screen progress bar and ratio
...
An agent can add or remove DOGFOOD story coverage and see the landing-screen progress update from real source data rather than manual copy edits.

## Human playback

1. A user launches DOGFOOD.
2. The landing screen shows the normal hero treatment.
3. The landing screen also shows a progress bar and ratio for current DOGFOOD documentation coverage.
...
4. It can verify that the landing copy and progress bar are driven from real story coverage data.
...
- the landing progress UI must remain secondary to the title screen, not overwhelm it
...
5. Render a progress bar and ratio on the landing screen.
...
- the landing surface shows the coverage ratio and percent
- the next DOGFOOD backlog item exists

## Risks

- choosing a denominator that feels arbitrary or magical
- adding too much landing chrome and weakening the title screen
...
- a landing progress bar and ratio that update from the current story set

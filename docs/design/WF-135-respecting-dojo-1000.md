...
`3,653` (`-1,287`). The stored ESLint baseline now records `3,237` live
findings, down from `4,517` (`-1,280`), and the next aggregate target is
`3,603` or lower. Follow-on passes removed fake async wrappers, dead DAG
fixtures/imports, explicit script/example formatting debt, MCP docs payload
parsing/coercion debt, flame typed-array non-null debt, and seven file/context
baseline entries while keeping touched file/context budgets below their stored
ceilings.

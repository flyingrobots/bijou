...
`2,558` (`-2,382`). The stored ESLint baseline now records `2,144` live
findings, down from `4,517` (`-2,373`), and the next aggregate target is
`2,508` or lower. Follow-on passes removed fake async wrappers, dead DAG
...
demo contract debt, while keeping touched file/context budgets below their
stored ceilings.

...
`2,399` (`-2,541`). The stored ESLint baseline now records `1,985` live
findings, down from `4,517` (`-2,532`), and the next aggregate target is
`2,349` or lower. Follow-on passes removed fake async wrappers, dead DAG

...
`2,944` (`-1,996`). The stored ESLint baseline now records `2,528` live
findings, down from `4,517` (`-1,989`), and the next aggregate target is
`2,894` or lower. Follow-on passes removed fake async wrappers, dead DAG
...
pipeline rejection cleanup, and seven file/context baseline entries while

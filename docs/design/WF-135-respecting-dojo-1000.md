...
`3,732` (`-1,208`). The stored ESLint baseline now records `3,316` live
findings, down from `4,517` (`-1,201`), and the next aggregate target is
`3,682` or lower. Follow-on passes removed fake async wrappers, dead DAG
fixtures/imports, explicit script/example formatting debt, and seven

...
human-facing docs surface for `4.1.0`. That means the release is still
blocked not only on missing corpus pages, but also on finishing the
shift away from `examples/` as a public docs and smoke authority.

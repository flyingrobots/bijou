...
| **Transition Shader API Normalization** | bijou-tui | Transition rendering is now surface-native, but `TransitionResult` still exposes text-era `char` / `charRole` naming for override semantics. Normalize the shader API around cell/override terminology and document surface-first shader authoring for v4. |
| **Example and Canary Bridge Burn-Down** | examples + repo tooling | The runtime contract is now pure `Surface | LayoutNode`, and the bridge residue is down to a small honest cluster: the `badge`/`theme`/`pipe` examples plus the showcase registry preview path where the badge component itself is the thing being demonstrated. Finish deleting incidental bridge use while keeping explicit text lowering only where the example is intentionally teaching that boundary. |
...
| **Structured Overlay Content API** | bijou-tui | The overlay family is surface-native internally, but the public `modal()` / `drawer()` / `tooltip()` content APIs are still mostly string-shaped. Add structured `Surface`/layout content inputs so overlays can host rich component content without flattening it first. |
...
| **Surface-Native Stack and Placement APIs** | bijou-tui | `flexSurface()` / `viewportSurface()` fixed the largest layout seam, but `vstack()`, `hstack()`, and `place()` still treat multiline text as the primary composition unit. Add a consistent surface-native stack/placement story and document when page/shell composition should stay structured instead of dropping back to string helpers. |
| **Surface-Native Pager and Focus Areas** | bijou-tui | `pager()` and string-first `focusArea()` still wrap text-oriented viewport helpers even though the underlying shell/runtime paths are now surface-native. Add structured companions or move these APIs onto `viewportSurface()` so scrollable panes with gutters/chrome no longer need to fall back to string bodies by default. |

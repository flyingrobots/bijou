...
| **Surface-Native Frame Shell Composition** | bijou-tui | The framed shell body path is now surface-native, but header/help chrome still runs through string styling before becoming surfaces and transition shaders still rely on text-grid bridging. Finish the shell cleanup so frame composition is surface-first end-to-end with text lowering only at explicit boundaries. |

...
| **Surface-Native Overlay Constructors** | bijou-tui | `modal()`, `toast()`, `drawer()`, and `tooltip()` are now surface-backed externally, but they still assemble strings internally before reparsing to `Surface`. Bring the overlay family into full design-system compliance by making surfaces the primary source of truth and keeping text lowering as an explicit edge behavior. |
| **Surface-Native Frame Shell Composition** | bijou-tui | `createFramedApp()` still composes parts of the shell chrome as strings before repainting them into surfaces. Move the frame header/help/body composition to an end-to-end surface-native path so the shell stops reintroducing string-era seams internally. |

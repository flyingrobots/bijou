...
| **Example and Canary Bridge Burn-Down** | examples + repo tooling | The runtime contract is now pure `Surface | LayoutNode`, but some examples and canaries still use explicit `surfaceToString()` and ANSI-to-surface bridge helpers at the edges. Early burn-down batches migrated line-oriented demos, overlay examples, prompt flows, helper demos, and the shared showcase/canary stack; keep deleting the remaining badge/string bridges as the example fleet catches up. |

...
| **Surface-Native Frame Shell Composition** | bijou-tui | The framed shell body path and transition rendering are now surface-native, but header/help chrome still runs through string styling before becoming surfaces. Finish the shell cleanup so frame composition is surface-first end-to-end with text lowering only at explicit boundaries. |

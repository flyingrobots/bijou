...
| **Example and Canary Bridge Burn-Down** | examples + repo tooling | The runtime contract is now pure `Surface | LayoutNode`, but some examples and canaries still use explicit `surfaceToString()` and ANSI-to-surface bridge helpers at the edges. The first burn-down batch migrated several line-oriented runtime demos; keep deleting the remaining badge/string bridges and explicit lowering seams as the example fleet catches up. |

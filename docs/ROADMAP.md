...
| **Surface-Native Layout and Viewport APIs** | bijou-tui | `flex()` and `viewport()` still center their APIs on strings, which forces some runtime examples to flatten otherwise structured content. Add surface/layout-aware variants so chat/workspace demos can stay fully surface-native instead of stopping at the layout boundary. |

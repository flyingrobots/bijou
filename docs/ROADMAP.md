...
| **Canonical Preference List Surface** | bijou + bijou-tui + docs | Promote the polished settings-row treatment discovered in the shell drawer into real Bijou componentry so DOGFOOD and future shell surfaces can use canonical preference rows instead of bespoke shell-local rendering. Persistence remains explicitly out of scope by default; the framework should support in-memory settings first and only add storage through an explicit port/adapter later. Initial design captured in [Preference Lists Belong to Bijou](strategy/preference-lists-belong-to-bijou.md) and [Preference List Surface spec](specs/preference-list-surface.spec.json). |
| **Bijou UX Doctrine** | docs + design system | Establish a north-star product doctrine for humane TUI UX: focus ownership, truthful controls, density rules, shell behavior, graceful lowering, and the “docs are the demo” standard. Initial design captured in [Bijou UX Doctrine](strategy/bijou-ux-doctrine.md). |
| **Accessibility and Assistive Modes** | docs + bijou + bijou-tui | Define what accessibility means beyond component-level `accessible` lowering, including linearized app experiences, low-vision/color-blind/reduced-motion posture, semantic clarity, and future audit/tooling direction. Initial design captured in [Accessibility and Assistive Modes](strategy/accessibility-and-assistive-modes.md). |
| **Localization and Bidirectionality** | docs + bijou + bijou-tui | Design locale-aware shell/content infrastructure early: localizable shell copy, locale formatting seams, logical `start`/`end` direction, and eventual RTL/BiDi-safe layout behavior. Initial design captured in [Localization and Bidirectionality](strategy/localization-and-bidirectionality.md). |
| **bijou-i18n Runtime + Tooling Packages** | future bijou-i18n + future bijou-i18n-tools + docs | Split localization into a real runtime package and a real workflow/tooling package: `bijou-i18n` for in-memory catalogs, direction, formatting, and runtime-safe references; `bijou-i18n-tools` for stale detection, spreadsheet import/export, pseudo-localization, and catalog compilation. Initial design captured in [bijou-i18n Should Be a Real Package](strategy/bijou-i18n-packages.md), [bijou-i18n Runtime spec](specs/bijou-i18n-runtime.spec.json), and [bijou-i18n Tools spec](specs/bijou-i18n-tools.spec.json). |
| **AI Explainability Standard** | docs + design system | Promote explicit `[AI]` labeling, rationale/evidence/next-action disclosure, and confidence-aware advisory posture into a canonical Bijou standard for AI-mediated surfaces. Initial design captured in [AI Explainability Standard](strategy/ai-explainability-standard.md). |
| **Content Guide** | docs + design system | Define Bijou’s voice, tone, and product-writing rules for shell chrome, empty states, confirmations, errors, notifications, accessible copy, and localized surfaces. Initial design captured in [Content Guide](strategy/content-guide.md). |
...
| **Canonical Blocks Library** | bijou + bijou-tui + docs | After component and doctrine work stabilizes, define reusable, precomposed UI blocks for common patterns such as app shells, onboarding flows, auth/login forms, and settings/inspection surfaces so builders can start from strong defaults instead of assembling everything from raw primitives. |
...
| **Smoke Pipeline Optimization Follow-ups** | repo tooling | The big structural speed wins are in: deterministic interactive coverage, local fast lane, redundant-build removal, and CI lane fan-out. If the suite is still too slow after those changes settle, the next options are artifact sharing across smoke jobs, finer sharding of the big static-tty lane, and further separation of local developer smoke from release-grade coverage. |

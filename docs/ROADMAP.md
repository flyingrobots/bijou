...
| ~~**Surface-Native Overlay Constructors**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. `modal()`, `toast()`, `drawer()`, and `tooltip()` now construct `Surface` objects directly and only derive text output as an explicit lowering path.~~ |
| ~~**Surface-Native Frame Shell Composition**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. The framed shell body path, transition rendering, and header/help chrome now compose directly from `Surface` data instead of routing shell structure through styled strings.~~ |
| ~~**Transition Shader API Normalization**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. `TransitionResult` now exposes `overrideChar` / `overrideCell` / `overrideRole`, and the package docs now document surface-first shader authoring instead of the old text-era naming.~~ |
| ~~**Example and Canary Bridge Burn-Down**~~ | ~~examples + repo tooling~~ | ~~Shipped on the v4 pure-surface branch. Examples now compose structured surfaces and lower once at explicit text endpoints, and the showcase preview contract now accepts `Surface` previews so even the badge showcase path no longer needs per-component bridge calls.~~ |
...
| **Component Guidance & Carbon-Style Usage Docs** | docs + examples + package READMEs | Audit the shipped component catalog and document what each semantic component is for, its variants, when to use it, when not to use it, whether it belongs in core or TUI-only, how it degrades across interaction layers, and its closest Carbon Design System analogue. |
| **Design-System Documentation Completeness** | docs | The component-family guide now has a structural completeness gate in CI (`docs:design-system:preflight`) and includes the required fields for every shipped family. Remaining work is deeper content guidance, stronger examples, and sharper edge-case policy rather than missing required sections. |
| ~~**Structured Overlay Content API**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. `modal()` / `drawer()` / `tooltip()` now accept plain strings or structured `Surface` content, so overlays can host rich rows and embedded component surfaces without flattening them first.~~ |
| ~~**Surface-Native Layout and Viewport APIs**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. `flexSurface()` and `viewportSurface()` now let runtime apps keep layout and scrolling on the `Surface` path instead of flattening structured content at the layout boundary.~~ |
| ~~**Surface-Native Stack and Placement APIs**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. `placeSurface()` now joins `vstackSurface()` / `hstackSurface()` as the structured layout trio, and the docs now explicitly steer rich TUI composition toward the surface-native path while leaving `vstack()` / `hstack()` / `place()` as explicit text-lowering helpers.~~ |
| ~~**Surface-Native Pager and Focus Areas**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. `pagerSurface()` plus the existing `focusAreaSurface()` / `createFocusAreaStateForSurface()` now keep scrollable panes on the structured `Surface` path, and the docs/examples now teach those as the canonical rich-TUI path while leaving `pager()` / `focusArea()` as explicit text-lowering helpers.~~ |
| ~~**Surface-Native Split Pane and Grid APIs**~~ | ~~bijou-tui~~ | ~~Shipped on the v4 pure-surface branch. `splitPaneSurface()` and `gridSurface()` now let workspace layouts stay structured all the way through pane and region composition, while `splitPane()` / `grid()` remain explicit text-lowering helpers.~~ |
| **Surface-Native Shell Utilities** | bijou-tui | `statusBar()` and the help renderers still expose text-first shell-chrome APIs even though the frame shell itself now composes structured surfaces. Backlog covers adding surface-native companions so shell chrome stops being the last obvious string-era teaching seam. |

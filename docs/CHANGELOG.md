...
- **Legends/Cycles workflow adopted** — Bijou now plans through legend docs, backlog items, cycle design docs, explicit invariant docs, and cycle-owned tests under `tests/cycles/<cycle>/`. `docs/specs/` is now explicitly frozen as a legacy planning surface for new work, and the fresh i18n package-boundary planning was rehomed into [LX-001 — bijou-i18n Runtime and Tooling Packages](BACKLOG/LX-001-bijou-i18n-runtime-and-tooling-packages.md) instead of continuing the old spec pattern.

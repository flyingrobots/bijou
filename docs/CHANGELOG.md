...
- **DOGFOOD landing refresh-rate readout** — the animated title screen now computes a live FPS value from the landing pulse cadence and displays it in the upper-left corner alongside the active landing performance tier, so the title treatment exposes both its current refresh behavior and the adaptive quality mode currently in effect.

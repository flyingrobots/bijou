...
- **WF-165 Code Dojo goalpost, tranche A** — The event bus, frame actions,
  ...
  optional error reporter cannot escape the reporting boundary. File/context
  debt falls from `37` to `32`, code-size debt falls from `25` to `20`, and
  aggregate debt falls from `62` to `52`; the active goalpost remains `12` or
  lower.
...
- **Focused Code Dojo ratchet** — WF-160 clears the final `6` ESLint findings,
  ...
  overrides, and removes `22` file/context entries through focused type and
  test-support extraction, lowering aggregate Code Dojo debt from `414` to
  `364` with the next goalpost target set to `314` or lower.

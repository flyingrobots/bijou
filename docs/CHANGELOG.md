...
- **Deterministic interactive example smoke path** — the core form layer now supports a semantic test-time key input port with raw-input fallback, interactive form examples export import-safe `main(...)` entrypoints, and `smoke:examples:all` now runs the interactive form lane in-process with test contexts instead of spawning PTYs and waiting on wall-clock step delays.
- **Faster local full-example smoke lane** — `smoke-all-examples` now supports `--fast`, `--skip-build`, and configurable pipe parallelism, and the workspace ships a new `npm run smoke:examples:fast` command so local example coverage no longer pays for the full release-grade build + static-TTY bar every time.
- **Release-grade smoke now skips redundant rebuilds** — CI, release readiness, release dry runs, and publish verification now call `smoke:examples:all -- --skip-build` after their earlier workspace build step, so the expensive release bars stop paying for the same full `tsc -b` pass twice.
- **Parallelized example smoke lanes for CI/release workflows** — the full example smoke runner now supports repeated `--mode=` filters, the workspace ships dedicated `smoke:examples:pipe`, `smoke:examples:static`, and `smoke:examples:interactive` commands, and the CI / release workflows now fan those lanes out as parallel matrix jobs instead of running one long monolithic example smoke step.

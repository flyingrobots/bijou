...
- **DF-001 honest DOGFOOD coverage progress** — DOGFOOD now has a dedicated [DF — DOGFOOD Field Guide](legends/DF-dogfood-field-guide.md) legend and a first cycle proving that the docs app can be honest about its own incompleteness. The landing screen now shows a coverage ratio and progress bar derived from the canonical [Component Families](design-system/component-families.md) reference plus the current DOGFOOD story set, and the next follow-on is explicitly tracked as [DF-002 — Expand DOGFOOD Component Family Coverage](BACKLOG/DF-002-expand-dogfood-component-family-coverage.md).

...
- **PR merge-readiness tooling** — `pr:review-status` now collapses to the latest non-automated review per reviewer, prefers GitHub author metadata for bot detection, treats `mergeStateStatus=UNKNOWN` as pending while GitHub computes mergeability, honors GitHub `reviewDecision` / `mergeStateStatus`, and down-ranks stale historical CodeRabbit rate-limit comments when a newer green bot signal exists; `pr:merge-readiness` adds a one-command merge gate summary on top of those signals.

...
- **LX-004 delimited sheet and JSON bundle adapters** — `@flyingrobots/bijou-i18n-tools` now ships the first concrete adapter layer on top of the LX-003 exchange seam: CSV and TSV serialization/parsing for `ExchangeSheet`, plus JSON serialization/parsing for `CatalogBundle`. The cycle is documented in [LX-004 — Provider Adapters for Workbook and Bundle Exchange](design/LX-004-provider-adapters-for-workbook-and-bundle-exchange.md), and the richer follow-on is parked as [LX-005 — Rich Spreadsheet and Filesystem Adapters](BACKLOG/LX-005-rich-spreadsheet-and-filesystem-adapters.md).

...
- **JSDoc fixes (bijou-tui, batch 2)** — fix 15 minor/nit issues: correct `matchesQuery` field check order, fix "Mutable" to "Immutable" on `TweenState`, add `@param`/`@returns` to `visibleLength` and `clipToWidth`, add `@returns` to `parseEntries` and `timeline()` and `run()`, add `@param`/`@returns` to `TimelineBuilder` and `Timeline` interface methods, add `@param`/`@returns` to `EventBus` interface methods, add `@throws` to `compile()`/`build()`, and standardize `--` to ` — ` (em-dash) in JSDoc across pager, flex, and timeline.

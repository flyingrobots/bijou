...
- **Animated Learn by Touch title screen** — the docs landing page now renders as pure animated art built from `assets/bijou.txt` and `assets/background.txt`, removing the old landing-page copy, stats, and CTA chrome so the first screen is just the shader-driven title treatment. The landing route also supports palette switching with `1-5` and `←` / `→`.

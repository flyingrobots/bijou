...
- **Respecting the Dojo burndown** — WF-135/WF-142 lower live type-aware ESLint
  findings from `4,517` to `1,206`, cutting `3,323` counted Code Dojo
  ...
  cleanup, perf overlay size formatting, theme builder/graph runtime guards,
  DAG source checked traversal, image-viewer app/test surface typing, and
  notification history formatting, DAG layout, flex layout, theme color/extend,
  panel dock, and surface layout checked-read cleanup. The aggregate debt
  ceiling now ratchets from `4,940` to `1,617`, with the next goalpost target
  set to `1,567` or lower, while touched file/context budgets remain held under
  their stored ceilings and two file/context exceptions are removed.
...
- **Safe GitHub comments and milestone item mirrors** —
  ...
  comments so review summaries preserve backticks, commands, and tables
  literally. `docs/ROADMAP.md` now labels release snapshot counts as milestone
  items and states that those totals include issues plus pull requests assigned
  to the milestone. This closes issues #287 and #288.
...
- **Status and feedback standard Blocks** —
  ...
  `ProgressIndicatorBlock`, with public lower-camel definitions
  `inlineStatusBlock`, `inFlowStatusBlock`, `transientOverlayBlock`,
  `activityStreamBlock`, `shortcutCueBlock`, and `progressIndicatorBlock`.
  The slice adds `inline` and `overlay` block scales, data contracts,
  schema-bound `*SchemaAdapter` and `*SchemaBlock` exports, command intents,
  ready story ids, standard catalog and package manifest entries, surface
  output for interactive/static modes, text output for pipe/accessible modes,
  and stable lowering facts for block identity, family, variant, selected
  identity, concrete mode, and semantic values. This moves issues #220 through
  #225 into explicit v6 block-catalog work instead of treating status feedback
  as a deferred v7 audit.
...
- **Frame-owned hosted runner for `createFramedApp()`** —
  ...
  entry point that defaults shell mouse support on. `@flyingrobots/bijou-node`
  `startApp(app)` now delegates to that hosted runner when present. The
  framed runner composes the shared runtime loop instead of forking it,
  uses the run-time `ctx` as the shell-owned rendering context, and folds
...
- **RE-008 byte-packed surface representation** — the internal `Surface` data structure is now backed by a packed `Uint8Array` (10 bytes per cell) instead of an array of heap-allocated `Cell` objects. Colors are stored as raw RGB bytes, modifiers as a flags bitfield, and characters as uint16 code points with a side table for multi-codepoint graphemes. The differ compares cells via 10-byte equality checks instead of string comparison, and emits ANSI SGR escape sequences directly from buffer bytes with a cached style lookup — bypassing chalk entirely on the hot diff path. Surface composition (`blit`) copies bytes directly between packed buffers. `fill()` and `clear()` use a template-stamp approach. A new `setRGB()` API writes numeric RGB values directly into the buffer with zero string parsing or object allocation. All components have been migrated to use `setRGB` when the surface is packed, with Cell-based fallbacks preserved. See the [RE-008 design doc](design/0001-008-byte-packed-surface-representation/008-byte-packed-surface-representation.md) for full details.
- **RE-017 byte-pipeline performance recovery** — full recovery of the render pipeline to pre-RE-008 performance and beyond. The diff path is now **32% faster on dense gradient frames** and **64% faster on sparse updates** vs the pre-cycle baseline. Key optimizations:
  - **Theme token color cache** — `Cell.fgRGB` / `Cell.bgRGB` fields let components skip per-cell hex parsing when theme tokens already carry pre-parsed RGB. The `tokenToCellStyle` helper threads them automatically for `createTextSurface` consumers. 1.9× paint speedup on synthetic theme-heavy surfaces.
  - **Render-dirty bitmap** — a second `Uint32Array` on `PackedSurface` tracks which cells were painted since the last `clear()`. The differ scans the bitmap union (target | current) word-at-a-time and skips cells that weren't touched in either surface. Unchanged frames return in near-zero time (**30× faster** on diff-static). Sparse updates (~10% dirty) are **1.9× faster**.
  - **Byte-native differ** — `renderDiffPacked` writes cursor moves, SGR prefixes, and cell characters as raw UTF-8 bytes directly into a pooled `Uint8Array` and hands them to `WritePort.writeBytes()` in one shot. No per-frame string concatenation, no `sgrCache` Map, no `String.fromCharCode` key allocation. The TUI runtime allocates the pooled buffer alongside the framebuffers and resizes it on terminal resize.
  - **Bench v2** — process-isolated, wall-time-only, scenario-agnostic benchmark harness (`bench/` workspace package) with 9 scenarios covering paint, diff, and realistic multi-region composition. CoVs <5% across all scenarios. See [bench/README.md](../bench/README.md).
  - Full audit and running log in [docs/perf/RE-017-byte-pipeline.md](perf/RE-017-byte-pipeline.md).
...
- **`PackedSurface` interface** — extends `Surface` with `.buffer` (the raw `Uint8Array`), `.sideTable` (grapheme cluster lookup), `.markAllDirty()`, `.markAllRenderClean()`, and `.renderDirtyWords` for buffer-copy and dirty-tracking workflows. Exported from the main barrel.
...
- **`WritePort.writeBytes(buf, len)`** — optional method for byte-level output. Node's `process.stdout.write(buf)` adapter implements it; the packed differ uses it to bypass string encoding on the hot path.
- **`RenderState.outBuf`** — the TUI runtime's rendering pipeline now carries a pooled byte buffer through all stages, available to the Diff middleware for direct byte emission.
...
- **METHOD migration** — backlog and legends migrated from `docs/method/graveyard/legacy-backlog/` to `docs/method/backlog/` with METHOD-compatible filenames. All 7 legends (DF, DL, DX, HT, LX, RE, WF) resolve correctly.
...
- **DX-001 typed framed-shell message surface** — `@flyingrobots/bijou-tui` now exports `FramePageMsg`, `FramePageUpdateResult`, `FramedAppMsg`, `FramedApp`, and `FramedAppUpdateResult`, and `createFramedApp()` now returns that explicit framed message surface instead of collapsing frame/page wrapper messages into plain `Msg`. Page updates now keep custom message unions typed while making raw `mouse` / `pulse` delivery explicit at the shell seam. The cycle is captured in [DX-001 — Type Framed App Messages and Updates End-to-End](design/DX-001-type-framed-app-messages-and-updates-end-to-end.md).
- **RE-006 component layout and interaction contracts** — `@flyingrobots/bijou-tui` now formalizes first-class component contracts inside the runtime-engine seam so retained layouts can carry explicit sizing/alignment rules, overflow ownership, and interaction participation metadata instead of treating components as anonymous rectangles. The runtime engine can now resolve the deepest enabled interactive node from a retained hit path, distinguish scroll-owning viewport components from ordinary pointer targets, and invoke component-level handlers that emit multiple commands and effects without bypassing the retained-layout model. The cycle is captured in [RE-006 — Formalize Component Layout and Interaction Contracts](design/RE-006-formalize-component-layout-and-interaction-contracts.md), and the next runtime-engine slice is now [RE-007](method/graveyard/legacy-backlog/up-next/RE-007-migrate-framed-shell-onto-runtime-engine-seams.md).
- **RE-005 buffered runtime commands and effects** — `@flyingrobots/bijou-tui` now formalizes first-class runtime command and effect buffers so layout-driven input routing no longer implies one input becomes one immediately-consumed message. The runtime-engine surface now supports buffering route outputs, appending multiple route results in order, applying buffered commands later in FIFO order, and executing buffered effects later in FIFO order while draining the corresponding buffer deterministically. The cycle is captured in [RE-005 — Buffer Commands and Effects Separately](design/RE-005-buffer-commands-and-effects-separately.md), and the next runtime-engine slices continue with [RE-006](design/RE-006-formalize-component-layout-and-interaction-contracts.md) and [RE-007](method/graveyard/legacy-backlog/up-next/RE-007-migrate-framed-shell-onto-runtime-engine-seams.md).
- **RE-004 layout-driven input routing and layer bubbling** — `@flyingrobots/bijou-tui` now exports a shell-agnostic runtime routing seam through `routeRuntimeInput(...)` plus retained-layout hit testing with `hitTestRuntimeLayout(...)` and explicit runtime input types for key and pointer events. The router visits the active view stack topmost-first, resolves pointer hits from retained geometry, stops at blocking views, and bubbles through non-blocking views while preserving separate `commands` and `effects` arrays in the routing result. The cycle is captured in [RE-004 — Route Input Through Layouts and Layer Bubbling](design/RE-004-route-input-through-layouts-and-layer-bubbling.md), and the next runtime-engine slices continue with [RE-005](design/RE-005-buffer-commands-and-effects-separately.md) through [RE-007](method/graveyard/legacy-backlog/up-next/RE-007-migrate-framed-shell-onto-runtime-engine-seams.md).
- **RE-003 retained layout trees and invalidation** — `@flyingrobots/bijou-tui` now exports a first-class retained-layout registry through `createRuntimeRetainedLayouts(...)`, `retainRuntimeLayout(...)`, `getRuntimeRetainedLayout(...)`, `listRuntimeRetainedLayouts(...)`, `invalidateRuntimeLayouts(...)`, and `dropInactiveRuntimeLayouts(...)`, plus an explicit invalidation-cause vocabulary for `terminal-resize`, `view-stack-change`, `content-change`, `visibility-change`, `enablement-change`, and `overflow-change`. This gives the runtime one shell-agnostic place to retain per-view layout trees, mark geometry stale without losing the last known tree, and drop geometry when views leave the stack. The cycle is captured in [RE-003 — Retain Layout Trees and Layout Invalidation](design/RE-003-retain-layout-trees-and-layout-invalidation.md), and the next runtime-engine slices continue with [RE-004](design/RE-004-route-input-through-layouts-and-layer-bubbling.md) through [RE-007](method/graveyard/legacy-backlog/up-next/RE-007-migrate-framed-shell-onto-runtime-engine-seams.md).
- **RE-002 first-class runtime state machine and view stack** — `@flyingrobots/bijou-tui` now exports a pure runtime-engine seam for explicit state and view ownership through `createRuntimeStateMachine(...)`, `transitionRuntimeState(...)`, `createRuntimeViewStack(...)`, `pushRuntimeView(...)`, `popRuntimeView(...)`, `replaceTopRuntimeView(...)`, `clearRuntimeViewsToRoot(...)`, and `replaceRuntimeRootView(...)`. The new runtime objects make root/base-view behavior explicit, keep state transitions distinct from view-stack transitions, and give future retained-layout and input-routing work a shell-agnostic foundation. The cycle is captured in [RE-002 — Promote First-Class State Machine and View Stack](design/RE-002-promote-first-class-state-machine-and-view-stack.md), and the remaining runtime-engine progression now continues with [RE-003](design/RE-003-retain-layout-trees-and-layout-invalidation.md) through [RE-007](method/graveyard/legacy-backlog/up-next/RE-007-migrate-framed-shell-onto-runtime-engine-seams.md).
- **RE-001 runtime-engine legend and architecture pass** — Bijou now has a dedicated [RE — Runtime Engine](legends/RE-runtime-engine.md) legend that formalizes the intended hexagonal runtime model around state machines, view stacks, retained layouts, layout-driven input routing, and distinct command/effect buffers. The cycle is captured in [RE-001 — Define Runtime Engine Architecture](design/RE-001-define-runtime-engine-architecture.md), adds explicit invariants for [State Machine and View Stack Are Distinct](invariants/state-machine-and-view-stack-are-distinct.md), [Layout Owns Interaction Geometry](invariants/layout-owns-interaction-geometry.md), and [Commands Change State, Effects Do Not](invariants/commands-change-state-effects-do-not.md), and decomposes the follow-on implementation work into [RE-002](design/RE-002-promote-first-class-state-machine-and-view-stack.md) through [RE-007](method/graveyard/legacy-backlog/up-next/RE-007-migrate-framed-shell-onto-runtime-engine-seams.md).
- **DF-014 through DF-019 complete DOGFOOD family coverage** — DOGFOOD now documents the remaining twelve canonical component families through real shell/workspace, navigation/progressive-disclosure, dense-comparison, hierarchy, temporal/dependency, branding/motion, transient-notification, and mode-aware authoring stories. The field guide now covers all `35/35` canonical families, raising honest coverage from `66%` (`23/35`) to `100%` (`35/35`), and the coverage ratchet now saturates cleanly at a `100%` floor with a `100%` next target instead of pretending there is another 5-point step after full coverage. The completion arc is documented in [DF-014 — Raise DOGFOOD Coverage Floor to 64%](design/DF-014-raise-dogfood-coverage-floor-to-64-percent.md), [DF-015 — Raise DOGFOOD Coverage Floor to 69%](design/DF-015-raise-dogfood-coverage-floor-to-69-percent.md), [DF-016 — Raise DOGFOOD Coverage Floor to 74%](design/DF-016-raise-dogfood-coverage-floor-to-74-percent.md), [DF-017 — Raise DOGFOOD Coverage Floor to 79%](design/DF-017-raise-dogfood-coverage-floor-to-79-percent.md), [DF-018 — Raise DOGFOOD Coverage Floor to 84%](design/DF-018-raise-dogfood-coverage-floor-to-84-percent.md), and [DF-019 — Raise DOGFOOD Coverage Floor to 100%](design/DF-019-raise-dogfood-coverage-floor-to-100-percent.md). The next DOGFOOD follow-on is now [DF-020 — Deepen DOGFOOD Story Depth and Variant Quality](method/graveyard/legacy-backlog/up-next/DF-020-deepen-dogfood-story-depth-and-variant-quality.md).
- **DF-013 shell-help and notification-system DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real help-view/hint stories and a real notification stack/history story, raising coverage from `60%` (`21/35`) to `66%` (`23/35`). The field guide now teaches shell-owned shortcut discovery and notification-system recall directly inside the docs app, the enforced floor moves up to `59%`, and the next explicit target is now `64%`. The cycle is documented in [DF-013 — Raise DOGFOOD Coverage Floor to 59%](design/DF-013-raise-dogfood-coverage-floor-to-59-percent.md), and the next ratchet is tracked as [DF-014 — Raise DOGFOOD Coverage Floor to 64%](design/DF-014-raise-dogfood-coverage-floor-to-64-percent.md).
- **DF-012 explainability and divider DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real `explainability()` and `separator() / separatorSurface()` stories, raising coverage from `54%` (`19/35`) to `60%` (`21/35`). The field guide now teaches calm recommendation walkthroughs plus honest section rhythm directly inside the docs app, the enforced floor moves up to `54%`, and the next explicit target is now `59%`. The cycle is documented in [DF-012 — Raise DOGFOOD Coverage Floor to 54%](design/DF-012-raise-dogfood-coverage-floor-to-54-percent.md), and the next ratchet is tracked as [DF-013 — Raise DOGFOOD Coverage Floor to 59%](method/graveyard/legacy-backlog/DF-013-raise-dogfood-coverage-floor-to-59-percent.md).
- **DF-011 text-entry and staged-form DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real `input() / textarea()` and `group() / wizard()` stories, raising coverage from `49%` (`17/35`) to `54%` (`19/35`). The field guide now teaches text entry plus grouped and staged form flows directly inside the docs app, the enforced floor moves up to `49%`, and the next explicit target is now `54%`. The cycle is documented in [DF-011 — Raise DOGFOOD Coverage Floor to 49%](design/DF-011-raise-dogfood-coverage-floor-to-49-percent.md), and the next ratchet is tracked as [DF-012 — Raise DOGFOOD Coverage Floor to 54%](method/graveyard/legacy-backlog/DF-012-raise-dogfood-coverage-floor-to-54-percent.md).
- **DF-010 single-choice and peer-navigation DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through a real `select() / filter()` story and a real `tabs()` story, raising coverage from `43%` (`15/35`) to `49%` (`17/35`). The field guide now teaches visible-list and searchable single-choice prompts plus peer navigation directly inside the docs app, the enforced floor moves up to `44%`, and the next explicit target is now `49%`. The cycle is documented in [DF-010 — Raise DOGFOOD Coverage Floor to 44%](design/DF-010-raise-dogfood-coverage-floor-to-44-percent.md), and the next ratchet is tracked as [DF-011 — Raise DOGFOOD Coverage Floor to 49%](design/DF-011-raise-dogfood-coverage-floor-to-49-percent.md).
- **DF-009 form DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real `confirm()` and `multiselect()` stories, raising coverage from `37%` (`13/35`) to `43%` (`15/35`). The field guide now teaches binary decisions and set-building prompts directly inside the docs app, the enforced floor moves up to `39%`, and the next explicit target is now `44%`. The cycle is documented in [DF-009 — Raise DOGFOOD Coverage Floor to 39%](design/DF-009-raise-dogfood-coverage-floor-to-39-percent.md), and the next ratchet is tracked as [DF-010 — Raise DOGFOOD Coverage Floor to 44%](design/DF-010-raise-dogfood-coverage-floor-to-44-percent.md).
- **DF-008 docs and reference DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real `markdown()` and `hyperlink()` stories, raising coverage from `31%` (`11/35`) to `37%` (`13/35`). The field guide now teaches bounded structured prose and explicit linked destinations directly inside the docs app, the enforced floor moves up to `34%`, and the next explicit target is now `39%`. The cycle is documented in [DF-008 — Raise DOGFOOD Coverage Floor to 34%](design/DF-008-raise-dogfood-coverage-floor-to-34-percent.md), and the next ratchet is tracked as [DF-009 — Raise DOGFOOD Coverage Floor to 39%](design/DF-009-raise-dogfood-coverage-floor-to-39-percent.md).
- **DF-007 feedback and history DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real `toast()` and `log()` stories, raising coverage from `26%` (`9/35`) to `31%` (`11/35`). The field guide now teaches low-level transient overlays separately from the notification system and gives activity streams their own honest component story, the enforced floor moves up to `29%`, and the next explicit target is now `34%`. The cycle is documented in [DF-007 — Raise DOGFOOD Coverage Floor to 29%](design/DF-007-raise-dogfood-coverage-floor-to-29-percent.md), and the next ratchet is tracked as [DF-008 — Raise DOGFOOD Coverage Floor to 34%](method/graveyard/legacy-backlog/DF-008-raise-dogfood-coverage-floor-to-34-percent.md).
- **DF-006 structural DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real `box()` and `inspector()` stories, raising coverage from `20%` (`7/35`) to `26%` (`9/35`). The field guide now teaches grouped containment and calm side-panel inspection directly, the enforced floor moves up to `24%`, and the next explicit target is now `29%`. The cycle is documented in [DF-006 — Raise DOGFOOD Coverage Floor to 24%](design/DF-006-raise-dogfood-coverage-floor-to-24-percent.md), and the next ratchet is tracked as [DF-007 — Raise DOGFOOD Coverage Floor to 29%](design/DF-007-raise-dogfood-coverage-floor-to-29-percent.md).
- **DF-005 DOGFOOD progress and loading coverage ratchet** — DOGFOOD now documents two additional canonical families through real `progressBar()` and `skeleton()` stories, raising coverage from `14%` (`5/35`) to `20%` (`7/35`). The progress story now includes both a static determinate preview and a looping pulse-driven rollout preview, the enforced floor moves up to `19%`, and the next explicit target is now `24%`. The cycle is documented in [DF-005 — Raise DOGFOOD Coverage Floor to 19%](design/DF-005-raise-dogfood-coverage-floor-to-19-percent.md), and the next ratchet is tracked as [DF-006 — Raise DOGFOOD Coverage Floor to 24%](design/DF-006-raise-dogfood-coverage-floor-to-24-percent.md).
- **DF-004 DOGFOOD coverage ratchet** — DOGFOOD now documents two additional canonical families through real `badge()` and `kbd()` stories, raising coverage from `9%` (`3/35`) to `14%` (`5/35`). The enforced floor and the CI/release gate moved up with that real content growth, and the next explicit target is now `19%`. The cycle is documented in [DF-004 — Raise DOGFOOD Coverage Floor to the Next 5-Point Target](design/DF-004-raise-dogfood-coverage-floor-to-next-target.md), and the next ratchet is tracked as [DF-005 — Raise DOGFOOD Coverage Floor to 19%](design/DF-005-raise-dogfood-coverage-floor-to-19-percent.md).
- **DF-003 DOGFOOD coverage floor gate** — DOGFOOD documentation coverage is now an enforced quality gate instead of only a displayed progress bar. The repo now has an explicit floor policy at the current honest 9%, an explicit next target at 14%, a `dogfood:coverage:gate` script, and CI/release-readiness wiring that fails if real story coverage drops below the floor. This makes the current state enforceable without pretending the docs are complete, and it keeps the next +5-point ratchet visible for future DF cycles. The cycle is documented in [DF-003 — Enforce DOGFOOD Coverage Floor](design/DF-003-enforce-dogfood-coverage-floor.md), and the next follow-on is [DF-004 — Raise DOGFOOD Coverage Floor to the Next 5-Point Target](method/graveyard/legacy-backlog/DF-004-raise-dogfood-coverage-floor-to-next-target.md).
- **HT-004 explicit layer objects and shell introspection** — `createFramedApp()` now promotes its shell/page layer model into richer explicit layer objects with stable ids, titles, footer-hint ownership, and help-source metadata. The public TUI API now also exports `activeFrameLayer()` and `underlyingFrameLayer()` alongside the richer `describeFrameLayerStack()` model, and the shell now consumes that same metadata for footer hints, help content, and shell-owned overlay titles instead of rediscovering them through separate branches. This closes the help-over-settings leak where keyboard help could fall back to workspace bindings even though settings was still the real underlying layer. The cycle is documented in [HT-004 — Promote Explicit Layer Objects and Richer Shell Introspection](design/HT-004-promote-explicit-layer-objects-and-richer-shell-introspection.md), and the follow-on is now tracked as [HT-005 — Promote Page-Provided Layer Registry and Shell Control Projection](method/graveyard/legacy-backlog/HT-005-promote-page-provided-layer-registry-and-shell-control-projection.md).
- **HT-003 layer-stack routing and footer truth** — `createFramedApp()` now routes shell input through an explicit topmost-layer model instead of relying only on raw branch order, and the public TUI API now exposes `describeFrameLayerStack()` so tests and agents can inspect the active workspace/modal/search/help/settings/notification/quit stack structurally. Mouse blocking and footer hints now respect that same topmost-layer model, which fixes the class of bugs where `Esc` or the gutter/footer could accidentally act on or advertise a lower layer. The cycle is documented in [HT-003 — Implement Layer Stack and Input Map Routing](design/HT-003-implement-layer-stack-and-input-map-routing.md), and its richer landed follow-on is now [HT-004 — Promote Explicit Layer Objects and Richer Shell Introspection](design/HT-004-promote-explicit-layer-objects-and-richer-shell-introspection.md).
- **HT-002 layered focus and interaction doctrine** — Bijou now has a dedicated Humane Terminal cycle for treating search, help, settings, notifications, quit confirm, and page modals as an explicit layer stack instead of ad hoc branch order. The cycle is captured in [HT-002 — Layered Focus and Interaction](design/HT-002-layered-focus-and-interaction.md), adds the new [Topmost Layer Dismisses First](invariants/topmost-layer-dismisses-first.md) invariant, and records the input-map direction: the topmost layer should drive both routing and visible footer/help controls. That doctrine is now backed by the landed [HT-003 — Implement Layer Stack and Input Map Routing](design/HT-003-implement-layer-stack-and-input-map-routing.md) cycle.
- **DF-001 honest DOGFOOD coverage progress** — DOGFOOD now has a dedicated [DF — DOGFOOD Field Guide](legends/DF-dogfood-field-guide.md) legend and a first cycle proving that the docs app can be honest about its own incompleteness. The initial docs content pane now shows a coverage ratio and progress bar derived from the canonical [Component Families](design-system/component-families.md) reference plus the current DOGFOOD story set, while the title screen now explains the `DOGFOOD` acronym instead of pretending documentation progress is a loading state. The next follow-on is explicitly tracked as [DF-002 — Expand DOGFOOD Component Family Coverage](method/graveyard/legacy-backlog/DF-002-expand-dogfood-component-family-coverage.md).
- **Prepared text layout seam for preference surfaces** — Bijou now has a reusable prepare/layout split for wrapped terminal text via `prepareWrappedText()` and `wrapPreparedTextToWidth()`, and the canonical preference-list/settings path now uses prepared rows and grapheme-aware width checks instead of recomputing string analysis from scratch. This gives long localized settings strings a more truthful layout foundation while borrowing the same “prepare once, layout cheaply” pattern we want to use more broadly.
- **Prepared text layout adoption in notification history** — the archived notification-history string renderer now prepares its title, metadata, message, and action fields before wrapping them, so the same reusable text-analysis seam now powers another review-heavy, localization-sensitive shell surface instead of living only inside preference rows.
- **Prepared render-entry reuse in the live notification stack** — the surface-native notification overlay path now prepares each card’s render entry once per pass and reuses it for viewport trimming, placement, and overflow-exit layout instead of rebuilding the same card geometry repeatedly. The stack behavior is unchanged, but the “prepare once, layout many” pattern now reaches the richer live shell overlays as well.
- **DL-006 inspector-panel rhythm proof** — Bijou now proves the inspector side of its design language through a canonical `inspector()` component in `@flyingrobots/bijou`, and DOGFOOD's variants pane now uses that surface instead of a bespoke detail box. The component makes the current selection more obvious than supporting copy, keeps compact titled sections calm, and lowers the same structure into pipe and accessible modes. The cycle is documented in [DL-006 — Prove Inspector Panel Rhythm](design/DL-006-prove-inspector-panel-rhythm.md), and the next follow-on is now tracked as [DL-007 — Promote Inspector Panel Block](method/graveyard/legacy-backlog/DL-007-promote-inspector-panel-block.md).
- **DL-007 inspector block promotion** — Bijou now promotes the inspector family into a real TUI block helper through `inspectorDrawer()` in `@flyingrobots/bijou-tui`. The helper composes shell-owned drawer behavior with canonical inspector content, uses a chrome-less inspector mode to avoid nested decorative borders, and is now proven in both the compact app-frame demo and the canonical release-workbench. The cycle is documented in [DL-007 — Promote Inspector Panel Block](design/DL-007-promote-inspector-panel-block.md), and the next follow-on is now tracked as [DL-008 — Promote Guided Flow Block](method/graveyard/legacy-backlog/DL-008-promote-guided-flow-block.md).
- **DL-005 guided-flow and explainability proof** — Bijou now proves the guided-flow side of its design language through a canonical `explainability()` component in `@flyingrobots/bijou`. The component marks AI visibly with `[AI]`, separates rationale, evidence, next action, and governance into calm sections, and lowers the same meaning into pipe and accessible modes instead of relying on visual chrome. The cycle is documented in [DL-005 — Prove Inspector and Guided Flow Rhythm](design/DL-005-prove-inspector-and-guided-flow-rhythm.md), and the next follow-on is now tracked as [DL-006 — Prove Inspector Panel Rhythm](method/graveyard/legacy-backlog/DL-006-prove-inspector-panel-rhythm.md).
- **DL-004 drawer rhythm and notice-row proof** — Bijou now proves calmer shell review-surface rhythm in the notification center instead of rendering that drawer as one large wrapped text blob. The notification review path now has structured surface-native notice rows with inset primary/supporting copy, and the app-frame notification drawer composes live and archived sections with clearer spacing and hierarchy. The cycle is documented in [DL-004 — Prove Drawer Rhythm and Notice Rows](design/DL-004-prove-drawer-rhythm-and-notice-rows.md), and the next follow-on is now tracked as [DL-005 — Prove Inspector and Guided Flow Rhythm](method/graveyard/legacy-backlog/DL-005-prove-inspector-and-guided-flow-rhythm.md).
- **DL-003 shared-surface pattern proof** — Bijou now proves its canonical selected-row treatment in shared TUI collection surfaces instead of only in preference lists and shell-local examples. `browsableListSurface()` and `commandPaletteSurface()` now use full-row background selection plus one-cell inset rhythm in the rich surface path, and the cycle is documented in [DL-003 — Prove Canonical Patterns in Shared Surfaces](design/DL-003-prove-canonical-patterns-in-shared-surfaces.md). The next follow-on is now tracked as [DL-004 — Prove Drawer Rhythm and Notice Rows](method/graveyard/legacy-backlog/DL-004-prove-drawer-rhythm-and-notice-rows.md).
- **DL-002 canonical patterns and blocks bridge** — Bijou’s design system now explicitly documents canonical selection, active-region, spacing, and overflow patterns in [Patterns](design-system/patterns.md), and it now has a first-class [Blocks](design-system/blocks.md) document that defines how reusable product-level assemblies should emerge from doctrine, patterns, and components. The cycle is documented in [DL-002 — Canonicalize Patterns and Blocks](design/DL-002-canonicalize-patterns-and-blocks.md), and the next follow-on is now tracked as [DL-003 — Prove Canonical Patterns in Shared Surfaces](method/graveyard/legacy-backlog/DL-003-prove-canonical-patterns-in-shared-surfaces.md).
- **DL-001 design language doctrine capture** — Bijou now has a dedicated [DL — Design Language](legends/DL-design-language.md) legend and a first completed doctrine cycle in [DL-001 — Capture Design Language Doctrine](design/DL-001-capture-design-language-doctrine.md). The doctrine layer now explicitly captures mouse posture, cognitive-load budgeting, accessibility’s semantic-shadow-model direction, localization’s catalog/reference/spreadsheet reality, `[AI]` explainability posture, and content guidance for both humans and agents. Notification-center polish is explicitly parked as [HT-001 — Notification Center Polish and Discoverability](method/graveyard/legacy-backlog/HT-001-notification-center-polish-and-discoverability.md), and the first follow-on design-language bridge has since landed as [DL-002 — Canonicalize Patterns and Blocks](design/DL-002-canonicalize-patterns-and-blocks.md).
- **LX-008 localized shell chrome and DOGFOOD proving surface** — `createFramedApp()` now accepts an i18n runtime and ships a canonical frame-shell catalog for command-palette, search, settings, help, quit, notification, and mode-label copy, while DOGFOOD now loads localized shell/docs catalogs for landing, onboarding, search, settings, and feedback copy. The cycle also proves one visible direction-aware shell behavior by resolving the settings drawer to logical `start` and the notification center to logical `end`, so RTL runtimes no longer leave every shell surface physically fixed. The cycle is documented in [LX-008 — Localize Shell Chrome and DOGFOOD](design/LX-008-localize-shell-chrome-and-dogfood.md), and the follow-on debt is now tracked as [LX-009 — Localize Shell Help, Notification, and Directional Surfaces](method/graveyard/legacy-backlog/LX-009-localize-shell-help-notification-and-directional-surfaces.md).
- **LX-001 `bijou-i18n` runtime package** — Bijou now ships a dedicated `@flyingrobots/bijou-i18n` workspace package for in-memory localization runtime concerns: namespaced catalogs, locale and direction metadata, message lookup with source-locale fallback, generic non-string resource lookup, runtime-safe references, and default Intl-based formatting seams. The cycle is documented in [LX-001 — bijou-i18n Runtime Package](design/LX-001-bijou-i18n-runtime-package.md), and the follow-on tooling half has since landed as [LX-002 — bijou-i18n Tools Package](design/LX-002-bijou-i18n-tools-package.md).
- **LX-002 `bijou-i18n-tools` package** — Bijou now ships a dedicated `@flyingrobots/bijou-i18n-tools` workspace package for localization workflow tooling: authoring catalogs, stable source hashing, automatic stale-translation detection, provider-neutral translation rows, row import/export, reference validation during compilation, runtime-catalog compilation, and pseudo-localization. The cycle is documented in [LX-002 — bijou-i18n Tools Package](design/LX-002-bijou-i18n-tools-package.md), and the exchange-layer follow-on has since landed as [LX-003 — Spreadsheet Adapters and Catalog Exchange Workflows](design/LX-003-spreadsheet-adapters-and-catalog-exchange-workflows.md).
- **LX-003 spreadsheet-friendly workbook and bundle exchange** — `@flyingrobots/bijou-i18n-tools` now ships a provider-neutral exchange layer on top of the LX-002 tooling core: typed value encoding/decoding, spreadsheet-friendly translation workbooks with string-cell-safe rows, serializable authoring-catalog bundles, workbook import back into translation rows, and bundle import back into authoring catalogs. The cycle is documented in [LX-003 — Spreadsheet Adapters and Catalog Exchange Workflows](design/LX-003-spreadsheet-adapters-and-catalog-exchange-workflows.md), and the first concrete adapters have since landed as [LX-004 — Provider Adapters for Workbook and Bundle Exchange](design/LX-004-provider-adapters-for-workbook-and-bundle-exchange.md).
- **LX-004 delimited sheet and JSON bundle adapters** — `@flyingrobots/bijou-i18n-tools` now ships the first concrete adapter layer on top of the LX-003 exchange seam: CSV and TSV serialization/parsing for `ExchangeSheet`, plus JSON serialization/parsing for `CatalogBundle`. The cycle is documented in [LX-004 — Provider Adapters for Workbook and Bundle Exchange](design/LX-004-provider-adapters-for-workbook-and-bundle-exchange.md), and the Node/filesystem follow-on has since landed as [LX-005 — Rich Spreadsheet and Filesystem Adapters](design/LX-005-rich-spreadsheet-and-filesystem-adapters.md).
- **LX-005 Node filesystem helpers for localization exchange** — Bijou now ships a dedicated `@flyingrobots/bijou-i18n-tools-node` workspace package on top of the LX-003/LX-004 exchange seam, with CSV/TSV sheet file helpers, JSON catalog bundle file helpers, workbook-directory read/write helpers with a versioned manifest, and extension-based format inference for `.csv`, `.tsv`, and `.json`. The cycle is documented in [LX-005 — Rich Spreadsheet and Filesystem Adapters](design/LX-005-rich-spreadsheet-and-filesystem-adapters.md), and the richer spreadsheet-provider follow-on has since landed as [LX-006 — XLSX Localization Adapters](design/LX-006-xlsx-localization-adapters.md).
- **LX-006 XLSX workbook adapters for localization exchange** — Bijou now ships a dedicated `@flyingrobots/bijou-i18n-tools-xlsx` workspace package on top of the LX-003 workbook seam, with `ExchangeWorkbook` -> XLSX byte serialization, XLSX byte parsing back into `ExchangeWorkbook`, explicit translation-workbook header validation, and deterministic workbook sheet-order preservation. The cycle is documented in [LX-006 — XLSX Localization Adapters](design/LX-006-xlsx-localization-adapters.md), and the service-backed follow-on is now parked as [LX-007 — Service-Oriented Localization Adapters](method/graveyard/legacy-backlog/LX-007-service-oriented-localization-adapters.md).
- **Shell-owned notification center implementation** — `createFramedApp()` now ships a right-edge notification center driven by the existing notification archive/history substrate, with a standard `Shift+N` shell binding, independent scroll/filter ownership, footer-level notification cues, command-palette entry, and the framed notification lab migrated off its custom history modal onto that shared shell surface.
- **Settings activation feedback** — shell-owned settings rows can now provide feedback copy, and `createFramedApp()` raises a standard toast when a setting is activated so preference changes feel acknowledged instead of silent. DOGFOOD now uses that path for visible settings like `Show hints` and `Landing quality`.
- **Stable shell settings toast widths** — notification records can now carry an explicit width hint, and the shell settings-feedback path uses a fixed toast width so rapid settings changes stack with consistent card widths instead of visually stepping as the copy changes.
- **Truthful footer hints and richer settings rows** — DOGFOOD now treats the footer as the active control surface instead of rendering stale pane legends inside page content, the footer only advertises the currently relevant pane-level hints when `Show hints` is enabled, and the shared shell settings drawer now renders row descriptions so discoverability copy lives in the shell itself instead of in ad hoc app chrome.
- **Landing-quality controls and stronger settings affordances** — the DOGFOOD landing screen now lets users adjust landing quality directly with `↑` / `↓` before entering the docs and raises immediate feedback toasts when that setting changes, while the shared shell settings drawer now gives toggle rows checkbox-style `☐` / `☑` affordances, marks choice rows distinctly, and lets apps describe what the current option actually means.
- **Shared DOGFOOD landing theme preference** — landing-theme changes now persist into the docs route instead of stopping at the title screen, DOGFOOD surfaces now expose the active theme inside the app itself, and the shell-owned settings drawer now includes a `Landing theme` choice row alongside `Landing quality`.
- **Settings drawer spacing and focus readability** — the shared shell settings drawer now uses one-cell breathing room between section headers, rows, and descriptions, focused rows render with a real background treatment instead of just a leading chevron, DOGFOOD groups theme controls under `Appearance` instead of the landing-only section, and DOGFOOD pane gutters now inherit the selected theme instead of keeping a hardcoded-feeling accent edge.
- **Settings focus backgrounds now respect surface background tokens** — focused shell settings rows now fill with the theme surface background channel instead of mistakenly using the token foreground color as a background, which fixes the blown-out white selection block that showed up in some palettes.
- **Settings rows now reflow long values and shell footer cues use a single `notices` label** — when a settings value would collide with its label, the shell now drops the value onto its own line instead of truncating it awkwardly, DOGFOOD no longer repeats the active theme as a separate content caption, and archived notification counts now use the same `notices` footer label as live notifications instead of introducing a mysterious `Inbox` term.
- **Canonical preference-list design direction captured** — recorded the next shell/design-system move explicitly: the settings drawer should keep its shell-owned behavior but promote its polished row treatment into canonical Bijou preference-list componentry, with persistence still intentionally out of scope unless supplied through an explicit port/adapter later.
- **Canonical preference-list component family now powers shell settings** — Bijou now ships `preferenceRowSurface()`, `preferenceListSurface()`, and `resolvePreferenceRowLayout()` as standard componentry for toggle/choice/info/action preference rows, and the frame-owned settings drawer now renders through that canonical family instead of hand-drawing a bespoke shell-only row treatment.
- **Product doctrine expansion for humane TUI UX** — added a new doctrine layer above shell/component docs: [Bijou UX Doctrine](strategy/bijou-ux-doctrine.md), [Accessibility and Assistive Modes](strategy/accessibility-and-assistive-modes.md), [Localization and Bidirectionality](strategy/localization-and-bidirectionality.md), [AI Explainability Standard](strategy/ai-explainability-standard.md), and [Content Guide](strategy/content-guide.md). The roadmap now also tracks these as first-class DOGFOOD/design-system work, plus a later canonical blocks library once the doctrine and component vocabulary stabilize.
- **Legends/Cycles workflow adopted** — Bijou now plans through legend docs, backlog items, cycle design docs, explicit invariant docs, and cycle-owned tests under `tests/cycles/<cycle>/`. `docs/specs/` is now explicitly frozen as a legacy planning surface for new work, and the first i18n work now lives in cycle/backlog artifacts instead of the old spec layer: [LX-001 — bijou-i18n Runtime Package](design/LX-001-bijou-i18n-runtime-package.md), [LX-002 — bijou-i18n Tools Package](design/LX-002-bijou-i18n-tools-package.md), [LX-003 — Spreadsheet Adapters and Catalog Exchange Workflows](design/LX-003-spreadsheet-adapters-and-catalog-exchange-workflows.md), [LX-004 — Provider Adapters for Workbook and Bundle Exchange](design/LX-004-provider-adapters-for-workbook-and-bundle-exchange.md), [LX-005 — Rich Spreadsheet and Filesystem Adapters](design/LX-005-rich-spreadsheet-and-filesystem-adapters.md), [LX-006 — XLSX Localization Adapters](design/LX-006-xlsx-localization-adapters.md), and [LX-007 — Service-Oriented Localization Adapters](method/graveyard/legacy-backlog/LX-007-service-oriented-localization-adapters.md).
- **DOGFOOD is now the release-facing smoke contract** — the workspace now ships `smoke:dogfood`, `smoke:dogfood:landing`, and `smoke:dogfood:docs`, driven through the deterministic `examples/docs/capture-main.ts` witness path instead of treating the broad `examples/` tree as the public smoke surface.
- **Release-grade DOGFOOD smoke now skips redundant rebuilds** — CI, release readiness, release dry runs, and publish verification now call `smoke:dogfood -- --skip-build` after their earlier workspace build step, so the release smoke path stops paying for the same full `tsc -b` pass twice.
- **Parallelized DOGFOOD smoke lanes for CI/release workflows** — the repo's release-facing smoke fanout now runs separate `landing` and `docs` DOGFOOD journeys in parallel matrix jobs instead of monolithically walking the old examples-first lanes.
- **Deferred smoke-speed follow-up note** — captured the remaining “if still too slow” ideas in the roadmap so we can return to humane-shell/product work without losing the next smoke-pipeline options: artifact sharing across jobs, finer sharding of the static-tty lane, and more aggressive separation between local and release-grade smoke coverage.

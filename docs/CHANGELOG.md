...
- **Dedicated bijou-i18n package direction captured** — added a concrete package-boundary follow-on for localization: [bijou-i18n Should Be a Real Package](strategy/bijou-i18n-packages.md), [bijou-i18n Runtime spec](specs/bijou-i18n-runtime.spec.json), and [bijou-i18n Tools spec](specs/bijou-i18n-tools.spec.json). The runtime/tooling split is now explicit: `bijou-i18n` owns in-memory catalogs, locale direction, formatting, and runtime-safe references, while `bijou-i18n-tools` owns authoring catalogs, stale detection, spreadsheet workflows, pseudo-localization, and compilation.

...
- **Example helper naming cleanup** — the shared example surface helpers no longer live behind the transition-era `examples/_shared/v3.ts` module name, so the example fleet stops teaching a V3-branded helper path while the repo moves toward V4-only guidance.

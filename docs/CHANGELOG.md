...
- **RE-005 buffered runtime commands and effects** — `@flyingrobots/bijou-tui` now formalizes first-class runtime command and effect buffers so layout-driven input routing no longer implies one input becomes one immediately-consumed message. The runtime-engine surface now supports buffering route outputs, appending multiple route results in order, applying buffered commands later in FIFO order, and executing buffered effects later in FIFO order while draining the corresponding buffer deterministically. The cycle is captured in [RE-005 — Buffer Commands and Effects Separately](design/RE-005-buffer-commands-and-effects-separately.md), and the next runtime-engine slices continue with [RE-006](BACKLOG/RE-006-formalize-component-layout-and-interaction-contracts.md) and [RE-007](BACKLOG/RE-007-migrate-framed-shell-onto-runtime-engine-seams.md).

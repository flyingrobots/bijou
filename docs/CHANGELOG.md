...
- **DF-001 honest DOGFOOD coverage progress** — DOGFOOD now has a dedicated [DF — DOGFOOD Field Guide](legends/DF-dogfood-field-guide.md) legend and a first cycle proving that the docs app can be honest about its own incompleteness. The initial docs content pane now shows a coverage ratio and progress bar derived from the canonical [Component Families](design-system/component-families.md) reference plus the current DOGFOOD story set, while the title screen now explains the `DOGFOOD` acronym instead of pretending documentation progress is a loading state. The next follow-on is explicitly tracked as [DF-002 — Expand DOGFOOD Component Family Coverage](BACKLOG/DF-002-expand-dogfood-component-family-coverage.md).
...
- **LX-006 XLSX workbook adapters for localization exchange** — Bijou now ships a dedicated `@flyingrobots/bijou-i18n-tools-xlsx` workspace package on top of the LX-003 workbook seam, with `ExchangeWorkbook` -> XLSX byte serialization, XLSX byte parsing back into `ExchangeWorkbook`, explicit translation-workbook header validation, and deterministic workbook sheet-order preservation. The cycle is documented in [LX-006 — XLSX Localization Adapters](design/LX-006-xlsx-localization-adapters.md), and the service-backed follow-on is now parked as [LX-007 — Service-Oriented Localization Adapters](BACKLOG/LX-007-service-oriented-localization-adapters.md).

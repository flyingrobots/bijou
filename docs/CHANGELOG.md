...
- **Low-allocation renderer Stage 1/2/3 (in progress)** — the interactive runtime now reuses front/back framebuffers instead of allocating and cloning a full-screen target surface every render, the core diff path now walks `surface.cells` directly instead of allocating masked cell copies through `get()` inside the hot loop, `Surface` clear/write operations now mutate existing cell objects in place instead of replacing them, and layout-view normalization now paints directly into its final target surface instead of allocating an intermediate content surface first. Against the checked-in renderer baseline on this machine, the current DOGFOOD landing benchmarks are materially better on the diff-heavy paths and large render sizes, with the strongest wins around `220x58` render+diff (`-11.0%`), `271x71` render (`-8.3%`), and `271x71` render+diff (`-7.9%`), while the smallest `220x58` render-only scenario remains noisy enough that it is still best treated as informational rather than a hard regression gate.
...
- **DOGFOOD title-screen render-path optimization** — the landing screen now precompiles theme color ramps, caches static text surfaces, precomputes background density rows, and renders the animated wave/logo fields through direct surface loops instead of generic per-cell shader/color parsing work, cutting the local benchmark from roughly `12.27 ms/frame` to `3.39 ms/frame` before terminal paint overhead.
- **Renderer benchmark harness** — the repo now ships reproducible DOGFOOD landing benchmarks plus comparison/guard commands, with a checked-in local-machine baseline snapshot so renderer/runtime work can measure before/after frame cost instead of relying on anecdotal terminal FPS.

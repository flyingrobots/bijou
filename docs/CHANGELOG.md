...
- **Low-allocation renderer Stage 1/2** — the interactive runtime now reuses front/back framebuffers instead of allocating and cloning a full-screen target surface every render, and the core diff path now walks `surface.cells` directly instead of allocating masked cell copies through `get()` inside the hot loop. Against the checked-in renderer baseline on this machine, the DOGFOOD landing benchmarks improved by about `9.6%` (`220x58` render), `11.5%` (`220x58` render+diff), `10.1%` (`271x71` render), and stayed roughly neutral on the largest `271x71` render+diff path.

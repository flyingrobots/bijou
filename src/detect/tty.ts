...
export function detectOutputMode(): OutputMode {
  if (process.env['BIJOU_ACCESSIBLE'] === '1') return 'accessible';

  if (process.env['NO_COLOR'] !== undefined) return 'pipe';
  if (process.env['TERM'] === 'dumb') return 'pipe';

  if (!process.stdout.isTTY) return 'pipe';

  if (process.env['CI'] !== undefined) return 'static';

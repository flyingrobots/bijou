...
describe('forms/types', () => {
  ...
  it('ValidationResult represents invalid state with message', () => {
    const result: ValidationResult = { valid: false, message: 'Too short' };
  ...
  it('FieldOptions type compiles', () => {
    const opts: FieldOptions<string> = {
      title: 'Name',
      defaultValue: 'John',
      required: true,
    };
  ...
  it('SelectOption type compiles', () => {
    const opt: SelectOption<number> = { label: 'One', value: 1, description: 'First' };
  ...
  it('ConfirmFieldOptions type compiles', () => {
    const opts: ConfirmFieldOptions = {
      title: 'Continue?',
      defaultValue: false,
    };
  ...
  it('GroupFieldResult type compiles', () => {
    const result: GroupFieldResult<{ name: string; age: number }> = {

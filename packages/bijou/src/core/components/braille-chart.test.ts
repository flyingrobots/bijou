...
describe('brailleChartSurface', () => {
  ...
  it('renders all-zero as blank braille (U+2800)', () => {
    const surface = brailleChartSurface([0, 0, 0, 0], { width: 2, height: 2, ctx });
    // All values the same → all at midpoint, bottom half filled
    for (let y = 0; y < surface.height; y++) {
      for (let x = 0; x < surface.width; x++) {
  ...
  it('bottom row has more dots than top row for ascending values', () => {
    ...
    // The single column samples two sub-pixels: left=0, right=100.
    ...
    // Bottom should have more bits set (more dots filled).

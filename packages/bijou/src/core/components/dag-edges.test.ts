...
import { encodeArrowPos, decodeArrowPos, junctionChar, createGrid, markEdge } from './dag-edges.js';
...
describe('markEdge', () => {
  it('marks straight vertical edge with correct directions', () => {
    ...
    markEdge(g, 0, 0, 0, 1, RS, colCenter);
    // sRow = 0*6+3 = 3, dRow = 1*6-1 = 5
    // Vertical from row 3 to row 4 (D,U), arrow at row 5
    expect(g.dirs[3]![5]!.has('D')).toBe(true);
    expect(g.dirs[3]![5]!.has('U')).toBe(true);
    expect(g.arrows.has(encodeArrowPos(5, 5))).toBe(true);
  });
  ...
  it('marks elbow edge with horizontal segment', () => {
    ...
    markEdge(g, 0, 0, 1, 1, RS, colCenter);

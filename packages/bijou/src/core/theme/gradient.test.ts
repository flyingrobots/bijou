...
describe('gradientText', () => {
  ...
  it('returns plain text when noColor is true', () => {
    expect(gradientText('hello', stops, { style, noColor: true })).toBe('hello');
  });

  it('returns empty string for empty input', () => {
    expect(gradientText('', stops, { style })).toBe('');
  });

  it('returns plain text for empty stops', () => {
    expect(gradientText('hello', [], { style })).toBe('hello');
  });

  it('preserves spaces and newlines', () => {
    const result = gradientText('a b', stops, { style });
    expect(result).toContain(' ');
  });

  it('contains the original characters regardless of color support', () => {
    const result = gradientText('ab', stops, { style });

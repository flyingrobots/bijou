...
describe('parseBCSS', () => {
  it('parses basic rules', () => {
    ...
    expect(must(sheet.rules[0]).declarations).toHaveLength(2);
  ...
  it('parses multiple selectors', () => {
    ...
    expect(must(sheet.rules[0]).selectors).toHaveLength(2);
  ...
  it('parses media queries', () => {
    ...
    expect(must(sheet.mediaQueries[0]).condition).toBe('(width < 80)');

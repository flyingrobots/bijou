...
const ALL_OUTPUT_MODES: readonly OutputMode[] = Object.freeze([
  'interactive',
  'static',
  'pipe',
  'accessible',
...
export const counterDemoBlock: BlockDefinition<CounterDemoBlockConfig, string> = defineBlock({
  metadata: {
    ...
    storyIds: ['fixture-counter-block'],
...
export function counterDemoLivePreviewText(
  model: CounterDemoModel,
  width: number,
  ctx: BijouContext,
): string {
  return [
    'Live example',
    indentBlock(surfaceToString(
      counterDemoBlockSurface(counterDemoBlockConfig(model, ctx, width)),
      ctx.style,
    )),
    '',
    'Live lowering preview',
    indentBlock(counterDemoLoweringPreviewText(model, width, ctx)),
    '',
    'Live documentation',
    indentBlock(counterDemoDocumentationText()),
  ].join('\n');
}
...
export function counterDemoLoweringPreviewText(
  ...
  return [
    'interactive:',
    indentBlock(counterDemoBlock.render({ config, mode: 'interactive' }).output),
    'static:',
    indentBlock(staticOutput),
    `pipe: ${pipeOutput}`,
...
export function counterDemoDocumentationText(): string {
  return [
    'CounterDemoBlock is a non-shipping fixture used by DOGFOOD and Storybook.',

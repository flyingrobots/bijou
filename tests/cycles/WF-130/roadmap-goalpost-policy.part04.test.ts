...
describe('WF-130 roadmap supporting contracts', () => {
  it('aligns bearing, release, design, and tracker lineage', () => {
    ...
    expectClaims(bearing, [
      ...
      'Tranche E must split `runtime-engine.ts`, `table.ts`, `app-frame.ts`, `examples/docs/app.ts`, and `examples/docs/stories.ts`, lowering the ledgers to `12` and `0`.',
      ...
      'Landed tranche D [#488](https://github.com/flyingrobots/bijou/pull/488) preserves `Surface`, DOGFOOD i18n-debt analysis, framed-app overlays, `ui-scene-ir/1`, and the terminal differ behind stable facades, lowering the live ceiling to `22`; five double-counted roots remain.',
      'Tranche E owns `runtime-engine.ts`, `table.ts`, `app-frame.ts`, `examples/docs/app.ts`, and `examples/docs/stories.ts`, targeting `12 + 0 = 12`.',

...
describe('WF-130 roadmap pull order', () => {
  it('binds the completed Dojo prerequisite and complete downstream order', () => {
    ...
    expectClaims(roadmap, [
      'Next Pull',
      '[#477](https://github.com/flyingrobots/bijou/issues/477) now owns the next `112 -> 62` goalpost.',
      'Its landed tranche A [#475](https://github.com/flyingrobots/bijou/pull/475) reduced measured debt to `87` by clearing `25` file/context violations.',
      'Tranche B must reach `62` or lower before [#468](https://github.com/flyingrobots/bijou/issues/468) can close.',
    ...
    expectOrderedClaims(bearing, [
      ...
      '1. Land the Code Dojo tranche-B goalpost in #477 at `62` aggregate debt or lower.',
      '2. Land the bounded Profunctor Page inspection proof in #468.',
      '3. Treat #458 as landed v8 foundation: the GraphQL block artifact bundle, replay facts, and visual scene facts are implemented.',
      '4. Pull #459 as the next v8 implementation proof: validate `packed-bijou-cells/1` and adapt it to `Surface`.',
      '5. Keep #302 in `v8.0.0` as the broad source tracker while landed #458 and active #459 prove the smallest stable contract.',

...
describe('DF-009 DOGFOOD form coverage cycle', () => {
  it('creates an active cycle doc with the required workflow sections', () => {
    const cycle = readRepoFile('docs/design/DF-009-raise-dogfood-coverage-floor-to-39-percent.md');
  ...
  it('spawns the next DOGFOOD backlog item', () => {
    expect(
      existsRepoPath('docs/BACKLOG/DF-010-raise-dogfood-coverage-floor-to-44-percent.md') ||
      existsRepoPath('docs/design/DF-010-raise-dogfood-coverage-floor-to-44-percent.md'),
    ).toBe(true);
  });
});

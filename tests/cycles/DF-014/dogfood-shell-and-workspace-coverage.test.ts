...
describe('DF-014 DOGFOOD shell and workspace coverage cycle', () => {
  it('creates an active cycle doc with the required workflow sections', () => {
    const cycle = readRepoFile('docs/design/DF-014-raise-dogfood-coverage-floor-to-64-percent.md');
  ...
  it('spawns the next DOGFOOD cycle', () => {
    expect(
      existsRepoPath('docs/BACKLOG/DF-015-raise-dogfood-coverage-floor-to-69-percent.md') ||
      existsRepoPath('docs/design/DF-015-raise-dogfood-coverage-floor-to-69-percent.md'),
    ).toBe(true);
  });
});
